...
if(!($fbid)){
	...
	values(DEFAULT,:fbid,:apn_id,:reg_id,:name,:email,'',:token,:gender,:profile_pic,:bio,YEAR(NOW())-YEAR(:dob),:city,:lat,:lang,NOW(),NOW())";
		...
		$sql="INSERT into user_setting(id,user_id,discovery,new_matches,push_notification,distance,interested_in,check_parameter,created_on) VALUES(DEFAULT,:user_id,1,1,1,2500,:interested_in,0,NOW())";
		$sth=$conn->prepare($sql);
		$sth->bindValue("user_id",$uid);
