               ...
               <!-- Category-->
                <section id="categorygrid">
                  ...
                  <ul class="thumbnails grid">
                   <?php  
  $pages = $setPerPage;
  $sqlCount =  "select count(*) FROM reward where status='1' AND instock>='1' ";
...
$searchSql = '';
  if(isset($_POST['search']) && $_POST['search']!=''){
        ...
        $listReward = Yii::app()->db->createCommand("SELECT a.*,b.image FROM  reward a 
          LEFT JOIN rewardimages b ON a.id=b.reward_id WHERE b.defaultimage='1' AND a.status='1' AND a.instock>='1'   $searchSql 
          ORDER BY a.$sort DESC limit $start, $setPerPage"
          )->queryAll();

            if(count($listReward) >0) {
             foreach($listReward as $data) {
               $explodeImg = explode('.',$data['image']);
               //echo "<pre>";
               //print_r($explodeImg);
               $getImage = $explodeImg['0']."m.".$explodeImg['1'];
              $filePath = Yii::app()->request->baseUrl."/images/reward/".$data['id']."/".$data['image'];
 ?>
                    ...
                    <li class="span3"> <a class="prdocutname" href="<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=reward/view&id=<?php echo $data['id']; ?>"><?php echo $data['title']; ?></a>
                      <div class="thumbnail">
                        ...
                        <a href="<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=reward/view&id=<?php echo $data['id']; ?>">
                          <img alt="image" src="<?php echo Yii::app()->request->baseUrl; ?>/images/reward/<?php echo $data['id']; ?>/<?php echo $getImage; ?>"> 
                          <?php //echo CHtml::image($filePath,'alt',array('width'=>270,'height'=>200));  ?>
                        ...
                        <div class="pricetag">
                          ...
                          <div class="price">
                            <div class="pricenew"><?php echo Yii::app()->params['adminCurrency']; ?><?php echo $data['price']; ?></div>
