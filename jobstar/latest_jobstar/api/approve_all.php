...
if(!($uid)){
	...
	*/
	...
	$WhereClause="AND DATE(job_status.created_on) BETWEEN  (UTC_DATE()-INTERVAL 1 DAY) AND (UTC_DATE()) AND job_status.is_completed=1 and job_status.inactive=0";
	...
	$sql="UPDATE `job_status` set is_approved=:complete_status,day=:day where kid_id 
	...
	UNION 
	select kids.id from users join `connect` on `connect`.`user_id2`=users.id left join kids on kids.id=`connect`.`kid_id` left join grownup_alias on grownup_alias.user_id=users.id and grownup_alias.kid_id=kids.id left join user_setting on user_setting.user_id=users.id where users.id={$uid}) $BonusjobClause ";	
