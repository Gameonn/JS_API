...
if(!($uid && $job_id && $kid_id && $day)){
	...
	//select * from `job_status` where job_id=167 and day='Monday' and kid_id=142 and DATE(created_on)=CURDATE()
	...
	$WhereClause="AND DATE(job_status.created_on) BETWEEN (UTC_DATE()-INTERVAL 1 DAY) AND (UTC_DATE())";
	
	//set particular job as complete/incomplete for the day
	$sql="UPDATE `job_status` set is_approved=:is_approved WHERE job_id=:id AND inactive=0 and is_completed=1 AND day=:day AND kid_id=:kid_id $WhereClause";	
