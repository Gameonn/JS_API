...
public static function getkidIdFromCode($buddy_code){

global $conn;
$sql="SELECT kid_id from buddyup WHERE buddy_code=:buddy_code";
$sth=$conn->prepare($sql);
...
//$path=BASE_PATH."timthumb.php?src=uploads/";
$path=BASE_PATH."uploads/";
$sql="SELECT users.id as uid,users.first_name as u_name,users.email,kids.id as kid,kids.name,kids.dob,(YEAR(NOW())-YEAR(kids.dob)) AS age1,(SELECT FLOOR(DATEDIFF( NOW(),kids.dob) / 365.25)) as age, kids.gender,kids.city,kids.country,kids.image FROM kids JOIN users on users.id=kids.user_id JOIN friends ON friends.kid_id2=kids.id WHERE friends.kid_id1=:kid_id 
UNION
SELECT users.id as uid,users.first_name as u_name,users.email,kids.id as kid,kids.name,kids.dob,(YEAR(NOW())-YEAR(kids.dob)) AS age1,(SELECT FLOOR(DATEDIFF( NOW(),kids.dob) / 365.25)) as age, kids.gender,kids.city,kids.country,kids.image FROM kids JOIN users on users.id=kids.user_id JOIN friends ON friends.kid_id1=kids.id WHERE friends.kid_id2=:kid_id";
$sth=$conn->prepare($sql);
$sth->bindValue("kid_id",$kid_id);
try{$sth->execute();}
catch(Exception $e){}
		...
		if(!ISSET($final[$value['kid']])){
			if($value['kid']){
				$final[]=array(
						...
						"user_id"=>$value['kid']?$value['kid']:"",
						"u_name"=>$value['u_name']?$value['u_name']:"",
...
public static function get_leaderboard($flag,$job_parameter,$country){
...
//flag condition
if($flag==1){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY - INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) - INTERVAL 7 DAY) ";
}
elseif($flag==2){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY)) ";
}
elseif($flag==3){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY + INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) + INTERVAL 7 DAY) ";
}
elseif($flag==4){
$WhereClause="AND DATE(job_status.created_on) BETWEEN '$d2' AND '$d3' ";
}
elseif($flag==5){
$WhereClause="AND DATE(job_status.created_on) BETWEEN '$d1' AND '$d4' ";
}
else{}

//job_parameter condition
if($job_parameter==2)
	$JobClause = "ORDER BY approved_jobs DESC";
elseif($job_parameter==3)
	$JobClause = "ORDER BY bonus_approved_jobs DESC";
elseif($job_parameter==4)
	$JobClause = "ORDER BY badge_won DESC";
else
	$JobClause = "ORDER BY completed_jobs DESC";

if($country)
	$CountryClause="AND kids.country='$country'";

	$sql="SELECT kids.*,(YEAR(NOW())-YEAR(kids.dob)) AS age1,(SELECT FLOOR(DATEDIFF( NOW(),kids.dob) / 365.25)) as age,
	(SELECT count(job_status.id) from job_status where job_status.kid_id=kids.id $WhereClause)as total_jobs,
	(SELECT count(job_status.id) FROM `job_status` JOIN jobs on jobs.id=job_status.job_id WHERE job_status.kid_id=kids.id 
	AND jobs.bonus_job=1 and job_status.is_approved=1 $WhereClause) as bonus_approved_jobs,
	(SELECT COUNT(badge.id) from badge WHERE badge.kid_id=kids.id) as badge_won,
	(SELECT count(job_status.id) from job_status where job_status.kid_id=kids.id and job_status.is_completed=1 $WhereClause)as completed_jobs,
	(SELECT count(job_status.id) from job_status where job_status.kid_id=kids.id and job_status.is_approved=1 $WhereClause)as approved_jobs 
	FROM `kids` join job_status on job_status.kid_id=kids.id WHERE kids.is_deleted=0 $CountryClause GROUP BY kids.id $JobClause ";
	...
	if(count($res)){
		...
		foreach($res as $key=>$value){
			$data[]=array(
				...
				"bonus_approved_jobs"=> $value['bonus_approved_jobs']?$value['bonus_approved_jobs']:"0",
				"badge_won"=> $value['badge_won']?$value['badge_won']:"0",
...
public static function getLinkedGrownups($kid_id){
...
if($flag==1){
$WhereClause="AND DATE(jobs.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY - INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) - INTERVAL 7 DAY) ";
$WhereClause1="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY - INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) - INTERVAL 7 DAY) ";
}
elseif($flag==2){
$WhereClause="AND DATE(jobs.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY)) ";
$WhereClause1="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY)) ";
}
elseif($flag==3){
$WhereClause="AND DATE(jobs.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY + INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) + INTERVAL 7 DAY) ";
$WhereClause1="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY + INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) + INTERVAL 7 DAY) ";
...
elseif($flag==4){
$WhereClause="AND DATE(jobs.created_on) BETWEEN '$d2' AND CURDATE() ";
$WhereClause1="AND DATE(job_status.created_on) BETWEEN '$d2' AND CURDATE() ";
}
elseif($flag==5){
$WhereClause="AND DATE(jobs.created_on) BETWEEN '$d1' AND CURDATE() ";
$WhereClause1="AND DATE(job_status.created_on) BETWEEN '$d1' AND CURDATE() ";
}
else{}
...
(SELECT COUNT(badge.id) from badge WHERE badge.kid_id=:kid_id) as badge, 
...
$sql="SELECT temp.* from (select users.id as uid,jobs.title,jobs.description,jobs.generic,jobs.bonus_job,jobs.repeat_job,job_status.day,
	...
	left join users as U on U.id=comments.user_id
	left join users as U1 on U1.id=likes.user_id
	where users.id=:user_id and jobs.is_deleted=0 and job_status.inactive=0
	UNION
	...
	left join users as U on U.id=comments.user_id
	left join users as U1 on U1.id=likes.user_id
	where users.id=:user_id and jobs.repeat_job=1 and jobs.is_deleted=0 and job_status.inactive=0
	UNION
	select users.id as uid,jobs.title,jobs.description,jobs.generic,jobs.bonus_job,jobs.repeat_job,job_status.day,job_status.is_completed,
	job_status.is_approved, job_status.id as js_id,jobs.created_on as job_date,jobs.id as jid,jobs.image as j_image,kids.name,kids.gender,kids.dob,
	kids.city,kids.country,kids.plan, kids.id as kid,kids.password as k_password, kids.image as k_image,(YEAR(NOW())-YEAR(kids.dob)) AS age1,(SELECT FLOOR(DATEDIFF( NOW(),kids.dob) / 365.25)) as age,
	...
	left join users as U on U.id=comments.user_id
	left join users as U1 on U1.id=likes.user_id
	WHERE users.id=:user_id and jobs.repeat_job=1 and jobs.is_deleted=0 and job_status.inactive=0) as temp ORDER BY temp.job_date DESC";
...
if($flag==1){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY - INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) - INTERVAL 7 DAY) ";
$where="AND DATE(job_status.created_on)< (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY - INTERVAL 7 DAY)";
}
elseif($flag==2){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY)) ";
$where="AND DATE(job_status.created_on)< (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY)";
}
elseif($flag==3){
$WhereClause="AND DATE(job_status.created_on) BETWEEN (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY + INTERVAL 7 DAY) AND (SELECT DATE_ADD((SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1)DAY),INTERVAL 7 DAY) + INTERVAL 7 DAY) ";
$where="AND DATE(job_status.created_on)< (SELECT CURDATE() - INTERVAL (WEEKDAY(CURDATE())+1) DAY + INTERVAL 7 DAY)";
...
elseif($flag==4){
$WhereClause="AND DATE(job_status.created_on) BETWEEN '$d2' AND '$d3' ";
$where="AND DATE(job_status.created_on)< '$d2' ";
}
elseif($flag==5){
$WhereClause="AND DATE(job_status.created_on) BETWEEN '$d1' AND '$d4' ";
$where="AND DATE(job_status.created_on)< '$d1' ";
}
else{}
