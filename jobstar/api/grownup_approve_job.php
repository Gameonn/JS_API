...
if(!($kid_id )){
	...
	/*if($flag==2){
	$WhereClause="AND Date_format(job_status.created_on,'%Y-%m-%d') BETWEEN (SELECT UTC_DATE() - INTERVAL (WEEKDAY(UTC_DATE())+1) DAY) AND (SELECT DATE_ADD((SELECT UTC_DATE() - INTERVAL (WEEKDAY(UTC_DATE())+1)DAY),INTERVAL 7 DAY)) ";
	}*/
