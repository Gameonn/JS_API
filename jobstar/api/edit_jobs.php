...
if(!($uid && $kid && $title && $days)){
	...
	$sql="DELETE from job_status where job_id=:job_id and kid_id=:kid_id and day NOT IN ($num) and job_status.created_on >=CURDATE()";
	$sth=$conn->prepare($sql);
	...
	$sql="INSERT INTO `jobs` (`id`, `user_id`, `kid_id`,`image`, `title`, `description`, `generic`, `bonus_job`, `repeat_job`, `created_on`,`updated_on`) VALUES (DEFAULT, :user_id, :kid_id,:image,:title,:description, 0, :bonus_job,:repeat_job,UTC_TIMESTAMP(),UTC_TIMESTAMP())";
