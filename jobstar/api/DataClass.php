...
global $conn;
$sql="select users.*,users.city as u_city,users.country as u_country,users.gender as u_gender,users.id as uid,users.image as u_image,kids.*,kids.id as kid,(YEAR(NOW())-YEAR(kids.dob)) AS age,kids.city as k_city,kids.country as k_country,kids.gender as k_gender,kids.image as k_image from `connect` join users on users.id=`connect`.user_id1 join kids on kids.id=`connect`.kid_id where code=:code ";
...
global $conn;
$sql="select users.*,users.city as u_city,users.country as u_country,users.gender as u_gender,users.id as uid,users.image as u_image,kids.*,kids.id as kid,(YEAR(NOW())-YEAR(kids.dob)) AS age,kids.city as k_city,kids.country as k_country,kids.gender as k_gender,kids.image as k_image from `friends` join kids on kids.id=`friends`.kid_id1 join users on users.id=kids.user_id where buddy_key=:code ";
...
public static function get_buddy_list($user_id){

global $conn;
$path=BASE_PATH."timthumb.php?src=uploads/";
$sql="SELECT users.id as uid,users.first_name,users.last_name,users.email,users.city as u_city,users.country as u_country,users.gender as u_gender, CONCAT('$path',users.image) as u_image,kids.id as kid,kids.name,kids.dob,(YEAR(NOW())-YEAR(kids.dob)) AS age, kids.gender,kids.city, kids.country,kids.image, friends.buddy_key, friends.status from users join kids on kids.user_id=users.id join friends on friends.kid_id2=kids.id where friends.kid_id1 IN (SELECT kids.id from kids where kids.user_id=:user_id)

UNION

SELECT users.id as uid,users.first_name,users.last_name,users.email,users.city as u_city,users.country as u_country,users.gender as u_gender, CONCAT('$path',users.image) as u_image,kids.id as kid,kids.name,kids.dob, (YEAR(NOW())-YEAR(kids.dob)) AS age ,kids.gender,kids.city, kids.country,kids.image,friends.buddy_key, friends.status from users join kids on kids.user_id=users.id join friends on friends.kid_id1=kids.id where friends.kid_id2 IN (SELECT kids.id from kids where kids.user_id=:user_id)";
$sth=$conn->prepare($sql);
...
$res=$sth->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach($res as $key=>$value){
		...
		$final[$value['uid']]=array(
		            ...
		            "image"=> $value['u_image']?$value['u_image']:"",
		            "kids"=>array()  
			);	
		}
...
global $conn;
$path=BASE_PATH."timthumb.php?src=uploads/";
$sql="select comments.id as comment_id,comments.comment,CONCAT('$path',users.image) as image, users.id as comment_user_id,users.first_name as name from users join comments on comments.user_id=users.id where comments.job_id=:job_id" ;
$sth=$conn->prepare($sql);
...
global $conn;
$path=BASE_PATH."timthumb.php?src=uploads/";
$sql="select likes.id as like_id,CONCAT('$path',users.image) as image, users.id as like_user_id,users.first_name as name from users join likes on likes.user_id=users.id where likes.job_id=:job_id" ;
$sth=$conn->prepare($sql);
$sth->bindValue("job_id",$job_id);
try{$sth->execute();}
catch(Exception $e){}
...
global $conn;
	$sql="select users.id as uid,users.city as u_city,kids.*,kids.id as kid,grownup_alias.alias,grownup_alias.access from users left join kids on kids.user_id=users.id left join grownup_alias on grownup_alias.user_id=users.id and grownup_alias.kid_id=kids.id left join user_setting on user_setting.user_id=users.id where users.id=:user_id
	UNION 
	select users.id as uid,users.city as u_city,kids.*,kids.id as kid,grownup_alias.alias,grownup_alias.access from users join `connect` on `connect`.`user_id2`=users.id left join kids on kids.id=`connect`.`kid_id` left join grownup_alias on grownup_alias.user_id=users.id and grownup_alias.kid_id=kids.id left join user_setting on user_setting.user_id=users.id where users.id=:user_id";
	$sth=$conn->prepare($sql);
...
global $conn;
	$sql="select kids.id from users join kids on kids.user_id=users.id left join grownup_alias on grownup_alias.user_id=users.id and grownup_alias.kid_id=kids.id left join user_setting on user_setting.user_id=users.id where users.id=:user_id
	UNION 
	select kids.id from users join `connect` on `connect`.`user_id2`=users.id left join kids on kids.id=`connect`.`kid_id` left join grownup_alias on grownup_alias.user_id=users.id and grownup_alias.kid_id=kids.id left join user_setting on user_setting.user_id=users.id where users.id=:user_id";
	$sth=$conn->prepare($sql);
	$sth->bindValue("user_id",$uid);
	try{$sth->execute();}
	catch(Exception $e){}
...
global $conn;
$sql="select users.*,users.id as uid,users.city as u_city,users.country as u_country,users.gender as u_gender,users.image as u_image,jobs.*,jobs.created_on as job_date,jobs.id as jid,jobs.image as j_image,kids.*,kids.id as kid,kids.password as k_password, kids.image as k_image,(YEAR(NOW())-YEAR(kids.dob)) AS age,user_setting.*,friends.*,`connect`.*,`connect`.id as cid,grownup_alias.*,comments.*,likes.*,comments.id as coid,likes.id as lid,U.first_name as c_name,U.image as c_image,U.id as udid,U1.first_name as l_name,U1.image as l_image,U1.id as ulid,(select count(likes.id) from likes where likes.job_id=jobs.id and likes.type='like') as likes_count,(select count(comments.id) from comments WHERE comments.job_id=jobs.id) as comment_count from users left join kids as k on k.user_id=users.id left join grownup_alias on grownup_alias.user_id=users.id left join user_setting on user_setting.user_id=users.id left join kids on  kids.user_id=users.id left join jobs on jobs.user_id=users.id and jobs.kid_id=kids.id left join friends on friends.kid_id1=kids.id left join `connect` on `connect`.user_id1=users.id or `connect`.`user_id2`=users.id left join comments on comments.job_id=jobs.id left join likes on likes.job_id=jobs.id
...
global $conn;
$sql="select users.id as uid,jobs.*,job_status.*,job_status.id as js_id,jobs.created_on as job_date,jobs.id as jid,jobs.image as j_image,kids.*,kids.id as kid,kids.password as k_password, kids.image as k_image,(YEAR(NOW())-YEAR(kids.dob)) AS age,grownup_alias.alias,grownup_alias.access,comments.*,likes.*,comments.id as coid,likes.id as lid,U.first_name as c_name,U.image as c_image,U.id as udid,U1.first_name as l_name,U1.image as l_image,U1.id as ulid,(select count(likes.id) from likes where likes.job_id=jobs.id and likes.type='like') as likes_count,(select count(comments.id) from comments WHERE comments.job_id=jobs.id) as comment_count from users left join kids  on kids.user_id=users.id left join grownup_alias on grownup_alias.user_id=users.id  left join jobs on jobs.user_id=users.id and jobs.kid_id=kids.id left join job_status on job_status.job_id=jobs.id left join comments on comments.job_id=jobs.id left join likes on likes.job_id=jobs.id
left join users as U on U.id=comments.user_id
left join users as U1 on U1.id=likes.user_id
where users.id=:user_id
UNION
select users.id as uid,jobs.*,job_status.*,job_status.id as js_id,jobs.created_on as job_date,jobs.id as jid,jobs.image as j_image,kids.*,kids.id as kid,kids.password as k_password, kids.image as k_image,(YEAR(NOW())-YEAR(kids.dob)) AS age,grownup_alias.alias,grownup_alias.access,comments.*,likes.*,comments.id as coid,likes.id as lid,U.first_name as c_name,U.image as c_image,U.id as udid,U1.first_name as l_name,U1.image as l_image,U1.id as ulid,(select count(likes.id) from likes where likes.job_id=jobs.id and likes.type='like') as likes_count,(select count(comments.id) from comments WHERE comments.job_id=jobs.id) as comment_count from users 
join `connect` on `connect`.`user_id2`=users.id 
left join kids on kids.id=`connect`.`kid_id`
left join grownup_alias on grownup_alias.user_id=users.id  
